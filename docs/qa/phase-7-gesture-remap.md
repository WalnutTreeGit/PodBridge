...
- **Available state** (driver present + AirPods Pro 2):
  - Experimental notice, first in the panel (verbatim): **`Experimental: current AirPods Pro 2 firmware often ignores this command. PodBridge sends your choice, but your AirPods may keep their previous press-and-hold action. Check on your AirPods whether it took effect.`** (issue #160 — the surface must not imply the change took effect)
  ...
  - **`Apply`** button
  - Persistence note (verbatim): **`Your choice is saved and sent again each time your AirPods reconnect — AirPods forget a PC's setting otherwise.`**
  - Apply result line (one of): **`Your AirPods acknowledged the change.`** (acknowledged) · **`Saved, but your AirPods didn't confirm it — current firmware often ignores this command, so it may not take effect. PodBridge will send it again the next time they reconnect.`** (no acknowledgement — the only branch observed on current firmware, #160) · **`The advanced-tier driver isn't available, so nothing was changed.`** (transport vanished)
...
- **Expected:** The window opens in the **Available** state, led by the **experimental notice** (`Experimental: current AirPods Pro 2 firmware often ignores this command…`), with the two per-bud dropdowns (each offering exactly `Cycle noise control` and `Siri / voice assistant`) and the persistence note. On **Apply**, the result line shows **`Your AirPods acknowledged the change.`** (the AirPods echoed the config within ~2 s). Pressing and holding each stem then performs the assigned action — the **left** does Siri, the **right** cycles noise control — i.e. the per-bud, right-then-left addressing is correct on real hardware. No UAC prompt at runtime (interface I/O needs no elevation). If the AirPods don't acknowledge, the line reads **`Saved, but your AirPods didn't confirm it — current firmware often ignores this command, so it may not take effect. PodBridge will send it again the next time they reconnect.`** — the choice is still persisted, which §5.9 exercises. **As of [#160](https://github.com/bhemsen/PodBridge/issues/160) this case FAILS on current firmware: the unacknowledged branch is the only one observed** (see §6).
...
- **Requires the Tier-2 advanced tier (a kernel driver).** Gesture remap only works when the opt-in Phase-6 driver is installed and loaded — which itself needs Secure Boot off + test-signing mode + a trusted test cert (three machine-wide, reversible security changes the user makes explicitly; PodBridge never runs `bcdedit`), and which cannot load at all while Memory Integrity is on. With the driver absent (the default) the feature is cleanly unavailable and Tier 1 is untouched. Production (attestation / EV) driver signing remains **DEFERRED** (Phase 6 note).
- **No CI hardware — apply + re-push are a manual smoke test.** CI runs Verify (build/format/unit) only; it cannot load the driver or exercise Bluetooth. So §5.7–§5.9 (assign, persist, reconnect-re-push on real AirPods Pro 2) are only ever verified by a human at the QA gate (constitution Tier-2 gate; `docs/workflow.md`).
- **§5.7 is FAIL on real hardware — gesture remap is experimental ([#160](https://github.com/bhemsen/PodBridge/issues/160)).** When the deferred `[real-hardware+driver]` cases were finally run on AirPods Pro 2 (Windows 11 x64 26100, test-signed driver), the ClickHoldMode `0x16` SET was **neither echoed nor applied**: the result line stayed on the "didn't confirm" branch and press-and-hold kept its previous action. The transport is **not** at fault — a noise-control SET over the same channel in the same session was echo-confirmed and audible. §5.9's re-push mechanism works but re-sends a frame the device ignores, so the earlier claim here (that an unconfirmed apply is "expected and recoverable — the stored choice re-applies on the next reconnect") does **not** hold on current firmware and has been withdrawn. Per #160 the frame stays **untouched** pending a clean-room re-derivation against current firmware — no guessed bytes are sent to the device. Until then the UI labels the feature experimental and claims only the send, never the effect.
